...
public class CardManager : MonoBehaviour
{
    ...
    [SerializeField] private List<Card> cards;   //ref to card sprites
    ...
    {
        var card = Instantiate(cardPrefab,transform.localScale,Quaternion.identity);
        card.name = cardData.name+cardData.suit;
        card.transform.SetParent(cardHolder.transform);
        card.GetComponent<CardView>().SetCardInfo(cardData);
    }
